 ...
 * @since 3.4.0
 */
final class ZipProtocolResolver implements ProtocolResolver {
    ...
    public Resource resolve(String location, ResourceLoader resourceLoader) {
        if (StringUtils.startsWithIgnoreCase(location, ZIP_PREFIX)) {

            var parts = StringUtils.delimitedListToStringArray(location.substring(ZIP_PREFIX.length()), "!");
            if (parts.length == 2) {
